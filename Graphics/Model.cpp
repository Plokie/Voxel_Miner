...
void Model::Init(ID3D11Device* device) {
	...
	/*CreateBuffer(D3D11_USAGE_DEFAULT, sizeof(Vertex) * ARRAYSIZE(vertices), D3D11_BIND_VERTEX_BUFFER, 0, &mesh->vertexBuffer, vertices);
	CreateBuffer(D3D11_USAGE_DEFAULT, sizeof(DWORD) * ARRAYSIZE(indices), D3D11_BIND_INDEX_BUFFER, 0, &mesh->indexBuffer, indices);*/
...
void Model::Draw(ID3D11DeviceContext* deviceCtx, XMMATRIX selfMx, XMMATRIX worldMx) {
